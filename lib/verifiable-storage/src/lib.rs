//! Verifiable Storage - Core traits for content-addressable storage using SAIDs.
//!
//! This crate provides the foundation for building verifiable, content-addressable
//! storage systems using Self-Addressing IDentifiers (SAIDs).
//!
//! # Core Concepts
//!
//! - **SAID** (Self-Addressing IDentifier): A content hash that serves as both
//!   the identifier and integrity check for data.
//! - **Chained**: Data with a stable prefix (lineage identifier) and
//!   cryptographic linking via previous pointers.
//!
//! # Traits
//!
//! - [`SelfAddressed`]: Types with a content-derived SAID
//! - [`Chained`]: Chained types with prefix and previous pointer
//! - [`ChainedRepository`]: Storage for chained types
//! - [`UnversionedRepository`]: Storage for simple SAID-addressed types

#![cfg_attr(
    test,
    allow(clippy::unwrap_used, clippy::expect_used, clippy::unwrap_in_result)
)]

mod error;
mod query;
mod repository;
mod said;
mod storable;
mod time;

pub use error::StorageError;
pub use query::{
    ColumnQuery, CorrelatedSubquery, Delete, Filter, Join, Order, OrderByTerm, Query,
    QueryExecutor, ScalarSubquery, TransactionExecutor, Value,
};
pub use repository::{
    ChainedRepository, ConnectionConfig, RepositoryConnection, UnchainedRepository,
};
pub use said::{
    Chained, SelfAddressed, compute_said, compute_said_for_fields, compute_said_from_value,
};
pub use storable::Storable;
pub use time::StorageDatetime;

// Re-export derive macro
// Note: SelfAddressed derive auto-detects versioning by presence of #[prefix], #[previous], #[version] fields
pub use verifiable_storage_derive::SelfAddressed;
